<?	// MODULE: CATEGORIES
	// FOR MBCMS ALL 'CATEGORIES' USED THROUGHOUT THE REST OF THE CMS
	...
	if($_GET['do'] == 'edit'){
		...
		if($_GET['category']){ 
			...
			$cname = mysql_result(mysql_query("SELECT `type` FROM `category` WHERE `id`='$category'"),0);
			echo "<p><b>".strtoupper($cname)." SUB CATEGORIES:</b></p>";
			$t = mysql_result(mysql_query("SELECT COUNT(id) FROM `category` WHERE `type`='$category'"),0);
			$q = mysql_query("SELECT * FROM `category` WHERE `type`='$category' ORDER BY `$ob` $or LIMIT $n, 15");
		...
		// VIEW LISTING OF ALL CATEGORIES
			...
			foreach($tables AS $k => $v){
				foreach($v AS $x){
					if(in_array('category', $x)){
						$qd['id'] = $tables[$k]['table'];
						$qd['title'] = $tables[$k]['name'];
						$qd['date'] = date('Y-m-d');
